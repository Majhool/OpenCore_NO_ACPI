  ...
  )
{
     ...
     && !UserSetMax)
  {
    if ((UserWidth == 0) || (UserHeight == 0)) {
      DEBUG ((DEBUG_WARN, "UEFI->Output->ConsoleMode 被破坏了，请查看文档!\n"));
      ++ErrorCount;
    } else if ((UserWidth < 80) || (UserHeight < 25)) {
      DEBUG ((DEBUG_WARN, "UEFI->Output->ConsoleMode低于最低支持的控制台文本分辨率80x25，请修复!\n"));
      ++ErrorCount;
    }
  }
  ...
  {
    DEBUG ((DEBUG_WARN, "UEFI->Output->Resolution 被破坏了，请查看文档!\n"));
    ++ErrorCount;
  }
